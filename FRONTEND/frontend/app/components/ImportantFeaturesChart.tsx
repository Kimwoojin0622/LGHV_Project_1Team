      ...
      try {
        ...
        const sortedData = response.data
          .map((item: any) => ({
            ...
            percentage: Math.round((Number(item.percentage) * 100) * 100) / 100, // 100을 곱하고 소수점 둘째자리까지 유지
          }))
                    ...
                    if (active && payload && payload.length) {
                      return (
                        <div className="bg-white p-2 border border-gray-200 rounded shadow-sm">
                          ...
                          <p className="text-sm font-bold" style={{ color: '#A50034' }}>영향도: {payload[0].value.toLocaleString()}</p>
                          {label === "결합 상품 유무" && (
